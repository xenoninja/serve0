// Package app runs the foreground preview command.
package app
...
// Run executes the command using the process streams and working directory.
// It serves until interrupted or an error occurs; version is the build identifier.
func Run(args []string, version string) error {
	if len(args) == 1 && args[0] == "--version" {
		fmt.Println("serve0", version)
		return nil
	}
